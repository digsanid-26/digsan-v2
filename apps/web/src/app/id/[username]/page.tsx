...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    publicTreeApi
      .getProfileByUsername(username, linkToken ?? undefined, tokens?.accessToken)
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center px-4">
          ...
          <div className="flex gap-3">
            ...
            {data?.family?.slug && (
              <Link href={`/family/${data.family.slug}`} className="text-white/40 hover:text-white text-sm">Kembali ke keluarga</Link>
            )}
          </div>
        </div>
      ) : data ? (
        <main className="flex-1 px-4 py-8">
          <Link
            href={data.family.slug ? `/family/${data.family.slug}` : '/'}
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden relative">
                ...
                <div className="px-6 pb-6 -mt-12">
                  ...
                  <div className="mt-5 flex flex-wrap gap-4 text-white/50 text-sm">
                    ...
                    {data.family.slug && (
                      <Link href={`/family/${data.family.slug}`} className="inline-flex items-center gap-1.5 text-blue-400 hover:underline">
