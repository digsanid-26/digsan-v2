...
export default function FamilyTreeVisual({ compact = false }: { compact?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    const showMemberDetail = (name: string) => {
      ...
      else if (name.includes('Kakak') || name.startsWith('KK')) relation = 'Saudara Tua';
      else if (name.includes('Adik') || name.startsWith('AD')) relation = 'Saudara Muda';
      else if (name.includes('Anak') || name.startsWith('AN')) relation = 'Keturunan';
    ...
    <div className="flex flex-col items-center">
      ...
      >
        ...
        <div
          ...
          style={{ left: '256px', top: '256px', width: '188px', height: '188px', opacity: 0, transition: 'opacity 0.45s ease' }}
        />
        ...
        {/* Lines */}
        <div id="line-top" className="tree-line absolute left-[349px] top-[80px] w-[2px] h-[180px] bg-white/50 rounded-full z-10" style={{ opacity: 0.15, transition: 'opacity 0.35s ease', boxShadow: '0 0 6px rgba(255,255,255,0.1)' }} />
        <div id="line-bottom" className="tree-line absolute left-[349px] top-[440px] w-[2px] h-[180px] bg-white/50 rounded-full z-10" style={{ opacity: 0.15, transition: 'opacity 0.35s ease', boxShadow: '0 0 6px rgba(255,255,255,0.1)' }} />
        <div id="line-left" className="tree-line absolute left-[80px] top-[349px] h-[2px] w-[180px] bg-white/50 rounded-full z-10" style={{ opacity: 0.15, transition: 'opacity 0.35s ease', boxShadow: '0 0 6px rgba(255,255,255,0.1)' }} />
        <div id="line-right" className="tree-line absolute left-[440px] top-[349px] h-[2px] w-[180px] bg-white/50 rounded-full z-10" style={{ opacity: 0.15, transition: 'opacity 0.35s ease', boxShadow: '0 0 6px rgba(255,255,255,0.1)' }} />

        {/* Peripheral Circles */}
        <div id="ot-circle" className="peripheral-circle absolute left-[320px] top-[55px] w-[60px] h-[60px] rounded-full flex items-center justify-center cursor-pointer z-30 border" style={{ opacity: 0.7, background: 'rgb(29,78,216)', borderColor: 'rgba(96,165,250,0.6)', boxShadow: '0 0 16px rgba(59,130,246,0.4)' }}>
          <span className="font-bold text-sm tracking-wider text-white">OT</span>
        </div>
        <div id="kk-circle" className="peripheral-circle absolute left-[55px] top-[320px] w-[60px] h-[60px] rounded-full flex items-center justify-center cursor-pointer z-30 border" style={{ opacity: 0.7, background: 'rgb(194,65,12)', borderColor: 'rgba(251,146,60,0.6)', boxShadow: '0 0 16px rgba(249,115,22,0.4)' }}>
          <span className="font-bold text-sm tracking-wider text-white">KK</span>
        </div>
        <div id="ad-circle" className="peripheral-circle absolute left-[585px] top-[320px] w-[60px] h-[60px] rounded-full flex items-center justify-center cursor-pointer z-30 border" style={{ opacity: 0.7, background: 'rgb(4,120,87)', borderColor: 'rgba(52,211,153,0.6)', boxShadow: '0 0 16px rgba(16,185,129,0.4)' }}>
          <span className="font-bold text-sm tracking-wider text-white">AD</span>
        </div>
        <div id="an-circle" className="peripheral-circle absolute left-[320px] top-[585px] w-[60px] h-[60px] rounded-full flex items-center justify-center cursor-pointer z-30 border" style={{ opacity: 0.7, background: 'rgb(180,83,9)', borderColor: 'rgba(251,191,36,0.6)', boxShadow: '0 0 16px rgba(245,158,11,0.4)' }}>
            ...
            <div className="glass border border-white/15 rounded-3xl overflow-hidden shadow-2xl">
              <div className="px-8 pt-7 pb-5 border-b border-white/[0.08]">
                ...
                <div className="flex bg-white/5 rounded-xl p-1">
                  <button
                    className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'login' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white/60'}`}
                    onClick={() => { setActiveTab('login'); setLoading(false); }}
                  >Masuk</button>
                  <button
                    className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'register' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white/60'}`}
                ...
                {activeTab === 'login' ? (
                  <form onSubmit={handleLogin} className="space-y-4">
                    <div>
                      ...
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nama@email.com" className="w-full bg-white/5 border border-white/10 focus:border-blue-400/50 rounded-xl px-4 py-2.5 text-white placeholder-white/25 text-sm outline-none transition-all" required />
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        <label className="text-[11px] font-medium text-white/50 tracking-wider uppercase">Kata Sandi</label>
                        <a href="/forgot-password" className="text-[11px] text-blue-400/80 hover:text-blue-300 transition-colors">Lupa kata sandi?</a>
                      </div>
                      <div className="relative">
                        <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full bg-white/5 border border-white/10 focus:border-blue-400/50 rounded-xl px-4 py-2.5 text-white placeholder-white/25 text-sm outline-none transition-all pr-10" required />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/60 transition-colors">
                          {showPassword ? <EyeOff size={15} /> : <Eye size={15} />}
                        </button>
                      </div>
                    </div>
                    <button type="submit" disabled={loading} className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold rounded-xl text-sm transition-all flex items-center justify-center gap-2 mt-1">
                  ...
                  <form onSubmit={handleRegister} className="space-y-3">
                    <div>
                      ...
                      <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nama Anda" className="w-full bg-white/5 border border-white/10 focus:border-blue-400/50 rounded-xl px-4 py-2.5 text-white placeholder-white/25 text-sm outline-none transition-all" required />
                    </div>
                    <div>
                      <label className="block text-[11px] font-medium text-white/50 mb-1.5 tracking-wider uppercase">Email</label>
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nama@email.com" className="w-full bg-white/5 border border-white/10 focus:border-blue-400/50 rounded-xl px-4 py-2.5 text-white placeholder-white/25 text-sm outline-none transition-all" required />
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 8 karakter" className="w-full bg-white/5 border border-white/10 focus:border-blue-400/50 rounded-xl px-4 py-2.5 text-white placeholder-white/25 text-sm outline-none transition-all pr-10" required />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/60 transition-colors">
                          {showPassword ? <EyeOff size={15} /> : <Eye size={15} />}
                        </button>
                      </div>
