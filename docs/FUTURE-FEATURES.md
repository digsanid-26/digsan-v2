...
- [x] Masalah -Akses terbatas Keluarga tidak ditemukan- belum teratasi. Error di console : Failed to load resource: the server responded with a status of 404 (Not Found). Mungkinkah karena fitur family node? Baik publik tree maupun publik user tidak bisa diakses (apakah slug berubah/sudah tidak bekerja?). Root cause: syncLinkedUser memanggil getOrCreateDefaultTree untuk linked user, yang bisa mengembalikan tree milik inviter (via FamilyMember lookup), lalu set slug=null pada tree inviter. Fix: gunakan getOrCreateOwnedTree + safety guard slug null hanya jika tree milik linked user. Tambah endpoint /trees/recover-slugs untuk memulihkan slug yang terhapus.
- [ ] Temukan cara aman yang minim konflik untuk membuat family node berfungsi. Tujuan awal Family Node adalah meringkas beberapa familymember menjadi kelompok dalam suatu wadah dan membentuk Tree Family Node. Mirip seperti Facebook Group namun memiliki type tingkatan : (1) Keluarga(terdiri atas suami, istri, anak-anak); (2) Keluarga Besar yang merupakan gabungan dari Keluarga yang masih dalam satu Orang tua baik dari pihak Suami maupun Istri, serta Keluarga-keluarga anak yang sudah berkeluarga, (3) Keluarga Simbah yang terdiri dari keluarga-keluarga yang lebih luas hingga ke kakek-nenek kedua pasangan, dst, dengan fitur-fitur seperti bisa memiliki halaman profil Family Node (image cover, background profil, informasi/deskripsi, dst), bisa menambah anggota baru atau menerima anggota baru (tombol gabung), memiliki fasilitas yang tumbuh seperti galeri, update aktivitas, share, like, komen, dst.
...
- [x] Lingkaran familymember perlu memiliki fitur untuk menambah, mengedit, dan menghapus anggota keluarga dalam bentuk icon kecil yang muncul pada saat sebuah lingkaran familymember dihover;
- [x] Icon + yang muncul di keempat sudut bagian luar lingkaran familymember (icon warna putih dibalut lingkaran warna hijau) yang berfungsi membuat lingkaran familymember baru sesuai posisi icon (misal klik icon + yang berada di kanan familymember akan menambah cabang/lingkaran familymember baru di sebelah kanan lingkaran familymember saat ini);
- [x] Tambahan tombol Hapus Profil di sidebar Detil Anggota (dibawah tombol edit profil);
...
- [x] (pembaharuan/tambahan) fitur role super_user yaitu bisa membuat lingkaran (familymember) unlimited dari lingkaran-lingkaran turunan di halaman app.digsan.id/tree dari misalnya familymember paman-pamannya (saudara ayah dan ibu) atau familymember kakak dan adiknya; 
- [x] Bisa membuat early access berupa pembuatan username (email) dan password untuk node yang dibuatnya, tidak terbatas pada anak-anak, pasangannya, serta kedua orang tua saja, tapi dari seluruh jaringan yang dibuatnya. Dia akan melihat form pembuatan akses (alamat email dan password) tersedia di sidebar detil anggota node/lingkaran yang dibuatnya;
- [x] Lingkaran family member yang telah dibuatkan early access akan otomatis memiliki publik member dan publik family tree sesuai di keluarga mana dia bergabung;
- [x] Role super_user bisa login familymember yang telah dibuatkan earlyaccess melalui halaman family/nama-keluarga yang dikelolanya. Pada modal yang muncul ketika node diklik terdapat tombol Login Early Access. 
- [x] Mungkin ada kesalahan persepsi, yang dimaksud early access adalah memungkinkannya seseorang login ke node yang dibuat super_user di masing-masing node user non-aktif menggunakan alamat email dan password sementara yang dibuat super_user untuk masing-masing node tersebut. Role super_user dapat membuat email dan password sementara tersebut di sidebar Detail Anggota tiap node yang dibuatnya, mungkin setelah section identifikasi anggota dan hanya bisa dilihat oleh role super_user. Login ke node user non-aktif tanpa perlu menempuh proses verifikasi akun dengan tujuan mengisi data profil general saja (seperti form di edit profil user) tanpa harus menempuh verifikasi akun atau mengaktifkan akun terlebih dahulu (misal untuk anak bayi/dibawah umur/sudah meninggal); 
